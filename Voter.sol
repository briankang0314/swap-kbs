...
contract Voter is IVoter {
    ...
    function createGauge(address _pool) external returns (address) {
        ...
        if (msg.sender != governor) { // gov can create for any pool, even non-Easy pairs
    ...
    function claimBribesWithtokenIds(address[] memory _bribes, address[][] memory _tokens, uint[] memory _tokenIds) external {
        
        for (uint i = 0; i < _tokenIds.length; i++){
        require(IVotingEscrow(_ve).isApprovedOrOwner(msg.sender, _tokenIds[i]));}

        for (uint i = 0; i < _tokenIds.length; i++){
        for (uint j = 0; j < _bribes.length; j++) {
            IBribe(_bribes[j]).getRewardForOwner(_tokenIds[i], _tokens[j]);}
        }
        }


    function claimFeesWithtokenIds(address[] memory _fees, address[][] memory _tokens, uint[] memory _tokenIds) external {
        
        for (uint i = 0; i < _tokenIds.length; i++){
        require(IVotingEscrow(_ve).isApprovedOrOwner(msg.sender, _tokenIds[i]));}
        
        for (uint i = 0; i < _tokenIds.length; i++){
        for (uint j = 0; j < _fees.length; j++) {
            IBribe(_fees[j]).getRewardForOwner(_tokenIds[i], _tokens[j]);
        }}
    }
