...
contract RewardsDistributor is IRewardsDistributor {
    ...
    function _checkpoint_total_supply() internal {
        ...
        for (uint i = 0; i < 20; i++) {
            if (t > rounded_timestamp) {
                ...
                int256 result = int256(pt.bias - pt.slope * dt);
                ve_supply[t] = result > 0 ? uint(result) : 0;
            }
            t += WEEK;
        }
        time_cursor = t;
    ...
    function _claim(uint _tokenId, address ve, uint _last_token_time) internal returns (uint) {
        ...
        for (uint i = 0; i < 50; i++) {
            ...
            if (week_cursor >= user_point.ts && user_epoch <= max_user_epoch) {
                ...
                uint balance_of = result > 0 ? uint(result) : 0;
    ...
    function claim(uint _tokenId) external returns (uint) {
        ...
        if (amount != 0) {
            bool lockEnded = IVotingEscrow(voting_escrow).lockEnded(_tokenId);

            if (lockEnded) {
            IVotingEscrow(voting_escrow).create_lock_for(amount, WEEK, msg.sender);
